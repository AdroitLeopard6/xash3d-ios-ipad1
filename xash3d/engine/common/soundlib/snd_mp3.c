...
/* Compatibility adapters for old libmpg callbacks on iOS ARMv7 */
static long XashMP3_Read( void *opaque, void *buffer, unsigned long size )
{
	file_t *file = (file_t *)opaque;
	return (long)FS_Read( file, buffer, (size_t)size );
}

static long XashMP3_Seek( void *opaque, long offset, int whence )
{
	file_t *file = (file_t *)opaque;
...
stream_t *Stream_OpenMPG( const char *filename )
{
	...
	if( !open_mpeg_stream( mpeg, file, XashMP3_Read, XashMP3_Seek, &sc ))
	{
#ifdef _DEBUG
