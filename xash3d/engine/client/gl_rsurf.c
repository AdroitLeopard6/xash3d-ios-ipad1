...
static void R_EnableBump()
{
	if( mtst.bump_enabled )
	{
		...
		pglTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), (void*)(const GLvoid *)offsetof( vbovertex_t, gl_tc ) );
		pglTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );

		// update deluxemap and bind normalmap
		GL_Bind( XASH_TEXTURE0, mtst.dl );
		pglTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), (void*)(const GLvoid *)offsetof( vbovertex_t, lm_tc ) );
